...
class PreferredSessionBrief(BaseModel):
    """希望セッションの表示に使う項目だけを返す（一覧の肥大化を防ぐ）"""
    id: int
    title: str
    start_time: datetime
    end_time: datetime
...
class StaffPreferredSessionResponse(BaseModel):
    ...
    session_id: int
    priority: int
    session: PreferredSessionBrief | None = None
...
class StaffBrief(BaseModel):
    """配置表示用の軽量スタッフ情報（希望セッション等の重い項目を含まない）"""
    id: int
    name: str
    photo: str
    english_ok: bool
...
class StaffScheduleBrief(BaseModel):
    """スタッフ別詳細用（活動可能時間まで含むが希望セッションは含まない）"""
    id: int
    name: str
    slack_name: str
    photo: str
