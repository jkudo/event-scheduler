...
PUBLIC_PATHS = {"/auth/login", "/auth/verify", "/auth/logout", "/auth/debug", "/auth/setup", "/auth/me"}
...
class AuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if session.get("role", "admin") == "viewer" and not viewer_allowed(request.method, path):
            return JSONResponse(
                status_code=403,
