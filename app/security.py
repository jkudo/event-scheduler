...
# 上限はワーカーごとに適用する。keep-alive で接続は1ワーカーに固定されるため、
# ワーカー数で割ると1クライアントから見た上限がその分だけ下がってしまう。
# 複数接続を張れば合計はワーカー数倍まで通るが、正規の利用を止める方が害が大きい。

# ログインは回数で絞らない。会場のWiFiは全員が同じグローバルIPになるため、
# IP単位で数えると他人の試行で締め出される。
# 総当たりに対しては PBKDF2（60万回）の計算コストで抑える。
...
# 同一IPの背後に会場の参加者全員がいる前提のため、閾値は大きく取る。
# 小さくすると、誰かの入力ミスで同じWiFiの全員がログインできなくなる。
LOGIN_LOCKOUT_THRESHOLD = 200  # failures within window
