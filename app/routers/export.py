...
def export_excel(db: Session = Depends(get_db)):
    ...
    for d in day_list:
        _fill_mgmt(wb.create_sheet(_safe_sheet_name(f"セッション管理{_sfx(d)}")), _subset(d))
    for d in day_list:
        _fill_place(wb.create_sheet(_safe_sheet_name(f"セッション配置{_sfx(d)}")), _subset(d))
    for d in day_list:
        _fill_matrix(wb.create_sheet(_safe_sheet_name(f"全体スケジュール{_sfx(d)}")), _subset(d))
    for cat_obj in db_categories:
        for d in day_list:
            subset = _subset(d)
            if not any(s.category == cat_obj.key for s in subset):
                continue
            _fill_cat(wb.create_sheet(_safe_sheet_name(f"{cat_obj.label}{_sfx(d)}")), cat_obj, subset)
