...
# 取り込み時の上限。無制限だと1回のアップロードでメモリを使い切れる
...
async def _read_limited(file: UploadFile, limit: int, label: str) -> bytes:
    ...
    while True:
        ...
        if total > limit:
            raise HTTPException(
                status_code=400,
                detail=f"{label}が大きすぎます。{limit // (1024 * 1024)}MB以下にしてください。",
            )
        chunks.append(chunk)
...
# 復元しても移行先の値を維持する設定。
# 認証情報のほか、data_version は応答キャッシュの世代管理に使う実行時の値で、
# 巻き戻すと更新前の内容を返し続けることがある
KEEP_SETTING_KEYS = ("reset_password", "login_password", "setup_completed", "data_version")
...
async def import_backup(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    raw = await _read_limited(file, MAX_BACKUP_BYTES, "バックアップ")
    buf = io.BytesIO(raw)

    # ZIP を展開して data.json を読み取る
    try:
        with zipfile.ZipFile(buf, "r") as zf:
            # 展開後の合計サイズを先に確認する（圧縮率を悪用した肥大化を防ぐ）
            total = sum(i.file_size for i in zf.infolist())
            if total > MAX_EXTRACTED_BYTES:
                return JSONResponse(status_code=400, content={"detail": "バックアップの展開後のサイズが大きすぎます"})
            ...
            for name in zf.namelist():
                if name.startswith("uploads/") and not name.endswith("/"):
                    # 画像以外を置くと /uploads/ から同一オリジンで配信されてしまう
    ...
    try:
        ...
        # ログイン/管理者パスワードは移行先の環境のものを維持する（復元でログインできなくなるのを防ぐ）
        db.query(AppSetting).filter(AppSetting.key.notin_(KEEP_SETTING_KEYS)).delete(synchronize_session=False)
        db.flush()
        ...
        # 画像を指す設定は復元後のファイル名に読み替える
        PATH_SETTING_KEYS = {"app_icon"}
        for s in data.get("settings", []):
            if s["key"] in KEEP_SETTING_KEYS:
                continue
            value = _map_path(s["value"]) if s["key"] in PATH_SETTING_KEYS else s["value"]
            existing = db.query(AppSetting).filter(AppSetting.key == s["key"]).first()
            if existing:
                existing.value = value
            else:
                db.add(AppSetting(key=s["key"], value=value))
...
def reset_all_data(body: ResetRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.query(VenueMap).delete()
        db.query(Room).delete()
        # uploads を消すので、画像を指す設定も一緒に消す
        db.query(AppSetting).filter(AppSetting.key.in_(["app_title", "app_icon"])).delete(synchronize_session=False)
        db.flush()
